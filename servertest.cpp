...
class Server {
 public:
  struct alignas(128) Client {
    std::atomic<int32_t> what; // starts as 0, which means nothing to do
      ...
      switch (nrClients) {
        case 1:
          for (size_t j = 0; j < 100; ++j) {
            int32_t what = clients[0]->what.load(std::memory_order_acquire);
            if (what > 0) {
              clients[0]->work->dowork();
              clients[0]->what.store(-what, std::memory_order_release);
            }
          }
          break;
        case 0:
...
void clientThread(Server* server, Work* work, std::atomic<int>* stop,
  ...
  while (stop->load() == 0) {
    for (size_t i = 0; i < perRound; ++i) {
      cl->what.store(2, std::memory_order_release);
